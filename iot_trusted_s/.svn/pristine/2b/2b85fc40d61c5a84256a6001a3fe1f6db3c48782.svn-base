...
status_t SPI_MasterTransferNonBlocking(SPI_Type *base, spi_master_handle_t *handle, spi_transfer_t *xfer)
{
    ...
    while (handle->state == kStatus_SPI_Busy)
    {
    	wait_count++;
    	if(wait_count > 2000000)
    	{
...
void SPI_MasterTransferHandleIRQ(SPI_Type *base, spi_master_handle_t *handle)
{
	...
	bool sendEnabled    = handle->txRemainingBytes;
	//需要为rxdata指定一个buffer
	while (((base->FIFOSTAT & SPI_FIFOSTAT_RXNOTEMPTY_MASK) && (base->FIFOINTENSET & SPI_FIFOINTENSET_RXLVL_MASK)) ||   //FIFO rx not emrty
		   (sendEnabled && (base->FIFOSTAT & SPI_FIFOSTAT_TXNOTFULL_MASK)))
	{
		/* Receive data */
		if ((base->FIFOSTAT & SPI_FIFOSTAT_RXNOTEMPTY_MASK) && (base->FIFOINTENSET & SPI_FIFOINTENSET_RXLVL_MASK))
		{
		...
		/* Send data */
		if (sendEnabled && (base->FIFOSTAT & SPI_FIFOSTAT_TXNOTFULL_MASK))
		{
			//base->FIFOWR = *handle->txData;
			SPI_WriteData(base,*handle->txData,0);
			handle->txRemainingBytes--;
			handle->txData++;
			num++;
			sendEnabled = handle->txRemainingBytes != 0;
			if (!sendEnabled)
			{
				...
				if (handle->callback)
				{
					handle->callback(base, handle, kStatus_SPI_Idle, 0);
				}
			}
		}
	}
