...
uint8_t* djsonConstruct(char* imei,float* temperature, char WE, float* longitude,char SN,float* latitude,uint8_t* GTime,uint8_t* LTime,int image,int button_light,int Cycle)
{
	...
	snprintf(dd,sizeof(dd),"{\"imei\":\"%s\",\"temperature\":%3.1f,\"GPS Longitude\":\"%c%6.3f\",\"GPS Latitude\":\"%c%6.3f\",\"GPS Time\":\"%s\",\"Local Time\":\"%s\",\"Image\":%d,\"button_light\":%d,\"Para Upload Cycle\":%d}",
			imei, *temperature,WE,*longitude,SN,*latitude,GTime, LTime,image,button_light,Cycle);
